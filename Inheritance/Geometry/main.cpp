...
namespace Geometry
{
	...
	{
	public:
		Triangle(Color color, int start_x, int strat_y, int line_width)
			:Shape(color, start_x,start_y, line_width){}
		~Triangle(){}
		virtual double get_height()const = 0;
		void info()const
		{
			cout << "Высота трейгольника: " << get_height() << endl;
			Shape::info();
		}
	};
	...
	public:
		...
		void set_side(double side)
		{
			if (side < MIN_SIZE)base = MIN_SIZE;
			if (side > MAX_SIZE)base = MAX_SIZE;
			this->side = side;
		}
		IsoscelesTriangle(double base, double side, Color colr, int start_x, int start_y, int line_width)
			:Triangle(color, start_x, start_y, line_width)
		{
			set_base(base);
			set_side(side);
...
void main()
{
	...
	Geometry::Square square(10, Geometry::Color::white, 300, 50, 5);
	square.info();

	Geometry::Rectangle rect(100, 75, Geometry::Color::blue, 450, 50, 8);
	rect.info();

	Geometry::Circle circle(50, Geometry::Color::yellow, 600, 10, 5);
	circle.info();

	Geometry::EquilateralTriangle equilateral_triangle(75, Geometry::Color::green, 400,400, 1);
	equilateral_triangle.info();

	Geometry::IsoscelesTriangle isosceles_triangle(100, 175, Geometry::Color::white, 800, 150, 1);
